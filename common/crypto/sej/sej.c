...
#include <types.h>
#include <mmio.h>
#include <crypto/rotate-bits.h>
#include <crypto/xor.h>
#include <security/sej.h>
#include <drivers/sgpt.h>

#define ERR_SEC_CYPHER_KEY_INVALID 0xC0020032
#define ERR_SEC_CYPHER_DATA_UNALIGNED 0xC0020033

/* MEID, BootROM uses these to generate it */

const u32 g_ME_ID_SRC[4] = {
    0xD485546A, 0xA91B0F4A, 0x4544FA47, 0x4BCE7924
};

const u32 g_ME_ID_IV[4] = {
    0xFF838E5C, 0xE940A0E3, 0x8D4DECC6, 0x45FC0989
};

const u32 g_ME_ID_SWOTP[8] = {
    0x51DD9914, 0x19DCE9F6, 0x35354683, 0xA093209F,
    0xAD915AD5, 0x6D69A6C6, 0x2C754B9A, 0x209B5244,
};

const u32 g_ME_ID_PATTERN[3][4] = {
    { 0xC9074D15, 0xA93744FB, 0x859ABD26, 0x50F3364C },
    { 0x8A854A81, 0xC5456832, 0xB43D3E86, 0x8924A523 },
    { 0x76C924A1, 0x52A38E62, 0x081E0B82, 0x870E3CF3 },
};

/* Anti clone */
const u32 g_AC_FIXED_PATTERN[3][4] = {
...
// "1A52A367CB12C458"
const u8 DEFAULT_SEED[32] = {
    0x31, 0x41, 0x35, 0x32, 0x41, 0x33, 0x36, 0x37,
    0x43, 0x42, 0x31, 0x32, 0x43, 0x34, 0x35, 0x38,
    0x39, 0x36, 0x35, 0x44, 0x33, 0x32, 0x43, 0x44,
    0x38, 0x37, 0x34, 0x42, 0x33, 0x36, 0x42, 0x32,
};
...
void sej_init(uintptr_t base_addr) {
    AES_IV iv;
    set_sej_base(base_addr);

    /* Clear Engine */
    u32 acon2_settings = readl(SEJ_ACON2);
    ...
    /* Init Secure GPT timer for style points */
    sej_gpt1_init(true);
}
...
int sej_set_key(AES_KEY_ID id,AES_KEY_SZ key) {
    extern void sej_aes_kdf(bool hrk256);
    ...
    u32 acon_settings = readl(SEJ_ACON);
...
u32 process_desc(sej_param_t desc) {
    u32 status = 0;
...
int sp_sej_enc(u8* buf, u8* out, sej_param_t desc) {
    ...
    if (desc.xor_en) {
        u32 xor_bytes = (desc.length < 0x10) ? desc.length : 0x10;
        xor_buf(buf, (const u8*)g_AC_BOOTROM_IV, xor_bytes, buf);
    }
...
int sp_sej_dec(u8* buf, u8* out, sej_param_t desc) {
    ...
    if (desc.xor_en) {
        u32 xor_bytes = (desc.length < 0x10) ? desc.length : 0x10;
        xor_buf(buf, (const u8*)g_AC_BOOTROM_IV, xor_bytes, out);
    }
...
u32 sej_prng(u32* iv, u32* out) {
    ...
    u32 rng_settings = SEJ_RNG_MOD_EN | SEJ_RNG_SRC_EN;
