...
void sej_aes_kdf(bool hrk256) {
    ...
    u32 timeout = sej_gpt1_time2tick_us(500);
    u32 start = sej_gpt1_get_tick();

    while (!(readl_relaxed(SEJ_ACON2) & SEJ_KDF_RDY)) {
        if (sej_gpt1_timeout_tick(start, timeout))
            return;
    }
...
void sej_aes_old_bk2c_key(void) {
    u32 aconk2_settings = readl(SEJ_ACONK2);
